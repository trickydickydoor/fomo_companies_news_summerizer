...
class GeminiService:
    ...
    def analyze_news(self, company_name: str, news_data: List[Dict[str, Any]], max_retries: int = 3) -> Dict[str, Any]:
        ...
        prompt = f"""
**重要：所有输出内容必须使用中文，包括所有字段名和内容**

请仔细分析以下新闻内容，**只提取与{company_name}公司直接相关的信息**。

**核心分析原则**：
1. **专注性**：只关注与{company_name}直接相关的内容，忽略新闻中的其他公司或话题
2. **相关性判断**：
   - 如果{company_name}是新闻的主角或有实质性内容，请详细分析
   - 如果{company_name}仅被简单提及（如在列表中、作为背景信息），请在facts中明确说明"仅被简单提及，无实质内容"

**内容要求**：
1. Topic应该明确描述与{company_name}相关的具体事件（而非宽泛的主题归类）
2. Aspect应该关注事件的具体细节和信息点
3. Content要包含与{company_name}相关的具体信息（人物、时间、地点、金额、产品等）
4. Citations提供充足的上下文，能够支撑Summary中提到的关键信息
5. 区分内容类型：
   - Facts（事实）：与{company_name}直接相关的具体事件、新产品发布、合作签约、财报数据等
   - Opinions（观点）：针对{company_name}的分析师评论、市场预期、专家观点、未来预测等
   - 忽略内容：纯历史回顾、仅提及公司名但无实质内容的新闻、与{company_name}无关的其他公司事件
6. 你收到了{len(news_data)}条相关内容，请确保不遗漏与{company_name}相关的重要事件，但不要强行分析无关内容

**重要提醒**：完全忽略与{company_name}无直接关系的新闻内容，即使这些内容很重要或占据新闻的主要篇幅。
